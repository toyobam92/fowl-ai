...
- **`awaiting_topic_pick`** → look in the inbox for an entry matching `^NOVA\s*(\d+|MORE)$` (case-insensitive, space between `NOVA` and the digit/`MORE` is optional — a reply like `NOVA3` with no space must still match). If none, exit. If found, remove it from the inbox (write back without it, commit as its own small commit — `git add automation/inbox.json && git commit -m "Drain inbox" && git push`), then go to step 2.
