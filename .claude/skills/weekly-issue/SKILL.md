...
- **`awaiting_pick`** → look in the inbox for an entry matching a digit 1-5 or `MORE` (case-insensitive); if none: when `last_updated` is 24+ hours old and `nudged_at` is unset, re-send the same numbered list via `notify.yml` prefixed with `Reminder — no pick yet, Monday's send is at risk:`, set `nudged_at` to now, commit+push the state file, and exit; otherwise just exit. One nudge per list only; clear `nudged_at` whenever `topics` is replaced (step 2's `MORE`) or the cycle resets to idle. (Added 2026-09-26: Issue 18's list got no reply for five days and missed its Monday slot.) If found, remove it from the inbox (write the file back without it, commit) and go to step 2.
...
Do a *light* research pass with `WebSearch` — enough to sketch 5 distinct plausible angles for this week's issue (not the full deep-dive yet). For each: a one-line title and a one-sentence why-now. Before choosing, read the `<title>` of the last 5-6 folders under `issues/` and make at least 3 of the 5 angles a genuinely different theme from those (policy/law, security, tools, agents, copyright, etc.), not another labor-market stat — Issue 18's first list was rejected wholesale as "the same theme" (2026-09-21). Write these to `automation/issue-state.json`:
